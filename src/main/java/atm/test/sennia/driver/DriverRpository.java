...
public interface DriverRpository extends JpaRepository<Driver,Integer> {
    @Query("SELECT d FROM Driver d WHERE d.id NOT IN (SELECT r.driver.id FROM Ride where r.state=:onGoing or r.state=:scheduled  )")
    public List<Driver> findAvaliableDrivers(@Param("onGoing") RideState onGoing, @Param("scheduled") RideState scheduled);
    @Query("SELECT CASE WHEN COUNT(r)>0 THEN false ELSE true END FROM Ride r WHERE r.driver.id=:driverId and (r.state=:onGoing or r.state=:scheduled) ")
