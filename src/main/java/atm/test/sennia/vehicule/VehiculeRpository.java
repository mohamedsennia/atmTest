...
public interface VehiculeRpository extends JpaRepository<Vehicule,String> {
    @Query("SELECT v FROM Vehicule v WHERE v.id NOT IN (SELECT r.vehicule.id FROM Ride where r.state=:onGoing or r.state=:scheduled  )")
    public List<Vehicule> findAvaliableVehicules(@Param("onGoing")RideState onGoing,@Param("scheduled") RideState scheduled);
    @Query("SELECT CASE WHEN COUNT(r)>0 THEN false ELSE true END FROM Ride r WHERE r.vehicule.id=:vehiculeId and (r.state=:onGoing or r.state=:scheduled) ")
